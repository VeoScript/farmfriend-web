...
import { useGetMonitoringLguNgo } from '@/helpers/tanstack/queries/monitoring/lgu-ngo'

const LguNgoTable = () => {

  const { data: lguNgo, isLoading } = useGetMonitoringLguNgo()
    ...
    <>
      {isLoading
        ...
        : <table className="w-full border-collapse border-b border-x border-slate-500 text-olive text-sm">
            ...
            <tbody>
              {lguNgo.map((lgu: { id: string, image: string, first_name: string, last_name: string, address: string, contact_num: string }) => (
                <tr key={lgu.id}>
                  ...
                  <td className="border border-slate-700 p-3">{ lgu.first_name + ' ' + lgu.last_name }</td>
                  <td className="border border-slate-700 p-3">{ lgu.address }</td>
                  <td className="border border-slate-700 p-3">{ lgu.contact_num }</td>
                </tr>
              ))}
            </tbody>
          </table>
